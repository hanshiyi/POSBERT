...
def load_and_cache_examples_features_tags(args, task, tokenizer, parser, evaluate=False):
    if args.local_rank not in [-1, 0] and not evaluate:
        torch.distributed.barrier()  # Make sure only the first process in distributed training process the dataset, and the others will use the cache
    ...
    cached_tags_file = os.path.join(args.data_dir, 'cached_{}_{}_{}_{}_{}'.format(
        'dev' if evaluate else 'train',
        list(filter(None, args.model_name_or_path.split('/'))).pop(),
        str(args.max_seq_length),
        str(task),
    ...
    if os.path.exists(cached_features_file) and os.path.exists(cached_tags_file) and not args.overwrite_cache:
        print("Loading features and tags from cached file %s and %s", cached_features_file, cached_tags_file)
        features = torch.load(cached_features_file)
        tags = torch.load(cached_tags_file)
        # print(type(features))
        # print(len(features))
        # print(len(features))
        # print(len(tags))
        ...
        features, tags = convert_examples_to_features_tags(examples,
                                                tokenizer,
                                                parser,
                                                label_list=label_list,
                                                max_length=args.max_seq_length,
        ...
        if args.local_rank in [-1, 0]:
            logger.info("Saving features into cached file %s", cached_features_file)
            torch.save(features, cached_features_file)
            torch.save(tags, cached_tags_file)
    ...
    all_input_ids = torch.tensor([f.input_ids for f in features], dtype=torch.long)
    all_input_tag_ids = torch.tensor([t.input_tag_ids for t in tags], dtype=torch.long)
    all_attention_mask = torch.tensor([f.attention_mask for f in features], dtype=torch.long)
    all_token_type_ids = torch.tensor([f.token_type_ids for f in features], dtype=torch.long)
    if output_mode == "classification":
        all_labels = torch.tensor([f.label for f in features], dtype=torch.long)
...
def evaluate_lex(args, model, tokenizer, parser, prefix=""):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        args.eval_batch_size = args.per_gpu_eval_batch_size * max(1, args.n_gpu)
        # Note that DistributedSampler samples randomly
        eval_sampler = SequentialSampler(eval_features_dataset)
        eval_dataloader = DataLoader(eval_features_dataset, sampler=eval_sampler, batch_size=args.eval_batch_size)
        ...
        # Eval!
        logger.info("***** Running evaluation {} *****".format(prefix))
        logger.info("  Num examples = %d", len(eval_features_dataset))
        logger.info("  Batch size = %d", args.eval_batch_size)
        eval_loss = 0.0
        nb_eval_steps = 0
        preds = None
...
def run_glue():
    ...
    tag_id_pickle_file = os.path.join(args.data_dir,"tag.pickle") # ).replace("train-", "").replace("dev-", "")
    tag_id_map = None
    ...
    lex_parser = Lex_parser(tag_id_initialized=tag_id_initialized, tag_id=tag_id_map)
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model = model_class.from_pretrained(checkpoint)
            model.to(args.device)
            result = evaluate_lex(args, model, tokenizer, lex_parser, prefix=prefix)
            result = dict((k + '_{}'.format(global_step), v) for k, v in result.items())
            results.update(result)
